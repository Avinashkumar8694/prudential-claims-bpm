...
```json
{
  ...
  "properties": {
    ...
    "caseStatus": {
      "type": "string",
      "enum": ["DRAFT", "SUBMITTED", "PENDING_DEATH", "PENDING_REQUIREMENTS", "PENDING_NIGO", "UNDER_REVIEW", "PARTIAL_REVIEW", "APPROVED", "PAYMENT_INITIATED", "PAYMENT_FAILED", "COMPLETED", "TERMINATED"],
...
## 📊 Case Lifecycle Statuses & Operational Triggers

The consolidated `caseStatus` field in the Claim Case Object acts as the primary indicator of the claim's operational state. The table below lists all allowed status values, the jBPM process nodes or API triggers that update them, and the exact business conditions for each transition:

| Status | Triggering Component / Node | Action / Event Description |
| :--- | :--- | :--- |
| **`DRAFT`** | Frontend Search API (`POST /caseservice/case`) | Created immediately when a claimant searches for policies and generates an initial quote. No documents or validations are locked yet. |
| **`SUBMITTED`** | Submit Claim API (`POST /api/v1/claims/submit`) | The claimant reviews and submits the completed web form. This triggers the jBPM process instance and binds the process instance ID (`jbpmProcessInstanceId`) to the case. |
| **`PENDING_DEATH`** | Main jBPM Process: Node 4 (`ServiceTask_SetPendDeath`) | Mainframe update. Policy status is set to "Pend Death" on the PAS mainframe for all associated policies, locking them from modifications. |
| **`PENDING_REQUIREMENTS`** | NIGO Subprocess: Node N1b (`_P2_N1B` status endpoint) | Triggered immediately after the NIGO notice is dispatched. Indicates the claim has open, missing requirements that must be resolved. |
| **`PENDING_NIGO`** | NIGO Subprocess: Wait Task (`_P2_N4`) | Suspends execution in the jBPM wait state, waiting for the claimant to upload the missing or incorrect documents. |
| **`UNDER_REVIEW`** | Main jBPM Process: User Tasks (`N13b: ExaminerReview` or `N16: ExaminerReview`) | Automated adjudication fails due to validation errors, contestability warnings, bank mismatches, or tax withholding exceptions. Claim escalates to a human examiner. |
| **`PARTIAL_REVIEW`** | NIGO Subprocess: Node `_P2_N9` (`ExaminerPartialReview`) | Reached if documents are uploaded and processed, but outstanding requirements remain, or if a TI claim NIGO loop times out. |
| **`APPROVED`** | Main jBPM Process: User Tasks completion | Triggered when the examiner successfully claims and completes the manual review task with an approval decision. |
| **`PAYMENT_INITIATED`** | Main jBPM Process: Node 22 (`ServiceTask_Finalize`) or Node 23 (`ServiceTask_KNECTPayment`) | Payout calculations are finalized and dispatched to payment gateways (ACH, WIRE, CHECK, or KNECT). |
| **`PAYMENT_FAILED`** | Main jBPM Process: Payment Gateway Error | Reached if the payment gateway rejects the transaction due to account invalidation or system errors (Scenario 11). |
| **`COMPLETED`** | Main jBPM Process: End Event | The payout transaction succeeds, payment references are bound to the beneficiaries, and the workflow terminates successfully. |
| **`TERMINATED`** | Main Process or Subprocess Error / Abort Path | The workflow is terminated. This occurs if bootstrap validation fails, the examiner manually aborts the claim, or the administrator initiates a termination. |
