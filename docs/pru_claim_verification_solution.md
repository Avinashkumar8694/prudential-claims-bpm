...
> The **Claim Verification** process (Track B). A verifier reviews an inbound **notification** in the workbench and decides **Promote / Hold / Close**. On Promote it generates the **Case ID** (claim ids are generated per-case downstream — a case has multiple claims) and hands off to the main claims pipeline (`pru-claim-processing`) as a **sub-process**.
>
...
| `verifierDecision` | String | **Output of the `Claims Verifier` user task** | `PROMOTE` \| `HOLD` \| `CLOSE`; drives the gateway |
| `verifierRemarks` | String | Output of the `Claims Verifier` user task | Free-text rationale; forwarded to promote/close APIs |
| `verifierUser` | String | Output of the `Claims Verifier` user task (completing user) | Used as `ActorId` to re-assign on the Hold loop-back |
| `notificationStatus` | String | **Set from API responses** (status / promote / close) | `FOR_VERIFICATION` → `VERIFIED_PROMOTED`/`ON_HOLD`/`NOT_VERIFIED_CLOSED` |
...
graph TD
    ...
    ST --> MG{merge}
    MG --> DC["Claims Verifier<br/>(User Task · Verifier group → auto-assigned; sets verifierDecision)"]
    ...
    GW -. "Hold (handled internally → back to same verifier)" .-> MG
...
> **Merge gateway before Claims Verifier.** A user task may have only **one** incoming connection, but two flows reach the verifier — the normal flow (from *Update Case Status*) and the **Hold** loop-back (from the decision gateway). A converging exclusive gateway (`merge`) joins them and feeds the single incoming flow into the Claims Verifier task.
...
| 2 | *merge* | Converging gateway | — | — | — |
| 3 | Claims Verifier | **User Task** (Verifier group — **auto-assigned**) | — | notification context (annotation) | `verifierDecision`, `verifierRemarks`, `verifierUser` |
| 4 | Verifier Decision? | Exclusive gateway | — | `verifierDecision` | — |
| 5 | Set Verified_Promoted + Generate Case ID | REST (POST) | `/v1/claims/verification/promote` | `notificationId`, `claimType`, `policyNumber`, `applicablePolicies`, `verifierRemarks` | `notificationStatus`, `caseId` (no claim id) |
| 6 | Send confirmation + claim forms | REST (POST) | `/v1/claims/verification/send-confirmation` | `caseId`, `applicablePolicies` | — |
| 7 | **System Claim Process** | **callActivity → `pru-claim-processing`** | — | 8 vars (see §6) | `caseId` (round-trip) |
| 8 | Set Not_Verified_Closed | REST (POST) | `/v1/claims/verification/close` | `notificationId`, `caseId`, `verifierRemarks` | `notificationStatus` |
| 9 | Generate and send Closure Notification | REST (POST) | `/v1/claims/verification/closure-notice` | `notificationId`, `caseId` | — |

The **Hold** branch has **no node** — the gateway's Hold flow loops straight back (via the merge) to the `Claims Verifier` task; the hold/status update is handled internally.

> **No "Assign Case to Verifier" step.** The `Claims Verifier` task is assigned to the `Verifier` group (`GroupId`), so it auto-lands on the verifier worklist (Alpha pull/auto-push, WB-4) — no explicit assignment service call is needed.
...
| Hold | `"HOLD".equals(verifierDecision)` | **loops straight back to `Claims Verifier`** (via merge; re-assigned to the same user via `ActorId`). No node — the hold/status update is handled internally |
