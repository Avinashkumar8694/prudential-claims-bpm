...
|---------------------|--------------------------------------------|--------------------|---------------------|---------|
| `FOR_VERIFICATION` | Update Case Status → `PUT /v1/claims/verification/case-status` | On process start, before assignment | Verifier inbox filter (→ `CASE_STATUS = For Verification`) | Notification is now on the verifier worklist |
| *(assigned)* | Assign Case to Verifier → `POST /v1/claims/verification/assign` | Right after status set | Sets `assignedTo` = verifier queue | Platform pull/auto-push abstraction |
| `VERIFIED_PROMOTED` | Promote → `POST /v1/claims/verification/promote` | Verifier chooses **Promote to Claim** | Generates `caseId` + `claimId`; hands off to **System Claim Process** | Track B equivalent of Track A Submit — formal claim begins (→ `CASE_STATUS = Claim Submitted`) |
| `ON_HOLD` | Retain Case → `PUT /v1/claims/verification/case-status` (status=`ON_HOLD`) | Verifier chooses **Hold** | Case retained; control **loops back to the `Claims Verifier` task**, re-assigned to the **same user** (via `ActorId`) | Reuses the case-status API (no separate `/hold` endpoint); the verifier keeps working the same case |
...
> **Promote hand-off.** On `VERIFIED_PROMOTED` the verification process invokes `prudential-claims-submission.pru-claim-processing` as a **call activity** (`System Claim Process`), passing `caseId`, `claimType`, `applicablePolicies`, and `cid` (the generated claim id). This is the entry point from Track B into the main internal-processing pipeline.
