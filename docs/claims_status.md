...
| `ON_HOLD` | Retain Case → `POST /v1/claims/status` (status=`ON_HOLD`) | Verifier chooses **Hold** | Case retained; control **loops back to the `Claims Verifier` task**, re-assigned to the **same user** (via `ActorId`) | Reuses the shared status API (no dedicated `/hold` or `/case-status` endpoint); the verifier keeps working the same case |
